...
class EncryptionClientGUI:
    ...
    def setup_ui(self):
        ...
        # Create frames for different functionality
        self.connection_frame = ttk.Frame(self.notebook, padding=10)
        self.document_frame = ttk.Frame(self.notebook, padding=10)
        self.search_frame = ttk.Frame(self.notebook, padding=10)
        self.session_frame = ttk.Frame(self.notebook, padding=10)
        ...
        self.setup_document_frame()
        self.setup_search_frame()
        self.setup_session_frame()
        self.setup_archive_frame()
    ...
    def setup_connection_frame(self):
        ...
        conn_group = ttk.LabelFrame(self.connection_frame, text="Server Connection", padding=10)
    ...
    def setup_document_frame(self):
        # Add document group
        add_group = ttk.LabelFrame(self.document_frame, text="Add Document", padding=10)
        ...
        # Document list group
        list_group = ttk.LabelFrame(self.document_frame, text="Your Documents", padding=10)
    ...
    def setup_search_frame(self):
        # Search group
        search_group = ttk.LabelFrame(self.search_frame, text="Keyword Search", padding=10)
        ...
        partial_group = ttk.LabelFrame(self.search_frame, text="Partial Keyword Search", padding=10)
        ...
        # Results group
        results_group = ttk.LabelFrame(self.search_frame, text="Search Results", padding=10)
    ...
    def setup_session_frame(self):
        # Session info
        info_group = ttk.LabelFrame(self.session_frame, text="Session Information", padding=10)
        ...
        # Session list
        list_group = ttk.LabelFrame(self.session_frame, text="All Sessions", padding=10)
        list_group.pack(fill=tk.BOTH, expand=True, pady=10)
    ...
    def setup_archive_frame(self):
        # Archive management
        archive_group = ttk.LabelFrame(self.archive_frame, text="Archive Access Management", padding=10)
        ...
        def connect_thread():
            try:
                ...
                if self.user_type.get() == "new":
                    messagebox.showinfo("Success", f"New user '{self.user_id.get()}' created successfully.")
                    ...
                    if messagebox.askyesno("Archive Passcode", "Would you like to set an archive passcode for accessing old sessions?"):
                        self.set_archive_passcode_dialog()
                else:
                    messagebox.showinfo("Success", f"Welcome back, {self.user_id.get()}!")
    ...
    def set_archive_passcode_dialog(self):
        ...
        def set_passcode():
            ...
            try:
                # For new users, we use the main passphrase as the "old" passcode
                self.sse.set_archive_passcode(self.passphrase.get(), passcode_entry.get())
    ...
    def set_archive_passcode(self):
        ...
        try:
            result = self.sse.db_manager.execute_query(
                "SELECT archive_key_hash FROM users WHERE user_id = ?",
                (self.sse.user_id,)
            )
