...
class ForwardPrivacySearchableEncryption:
    ...
    def _get_or_create_user(self) -> tuple[bytes, bytes]:
        ...
        result = self.db_manager.execute_query(
            "SELECT salt, key_encryption_key, archive_key_hash FROM users WHERE user_id = ?",
            (self.user_id,)
        )
    ...
    def _start_new_session(self):
        ...
        # Store session
        self.db_manager.execute_query(
            "INSERT INTO sessions (session_id, user_id, session_key_encrypted, created_at) VALUES (?, ?, ?, ?)",
            (self.current_session_id, self.user_id, encrypted_session_key, int(time.time()))
        )
        
        # Update user's current session
        self.db_manager.execute_query(
            "UPDATE users SET current_session_id = ? WHERE user_id = ?",
    ...
    def set_archive_passcode(self, old_passcode: Optional[str], new_passcode: str):
        ...
        # Store the hash
        self.db_manager.execute_query(
            "UPDATE users SET archive_key_hash = ? WHERE user_id = ?",
    ...
    def _get_session_key(self, session_id: str) -> Optional[bytes]:
        ...
        result = self.db_manager.execute_query(
            "SELECT session_key_encrypted FROM sessions WHERE session_id = ? AND user_id = ?",
    ...
    def add_document(self, doc_id: str, content: str, keywords: List[str]):
        ...
        try:
            ...
            self.db_manager.execute_query(
                "INSERT INTO documents (doc_id, session_id, content, keyword_hashes, created_at) VALUES (?, ?, ?, ?, ?)",
                (doc_id, self.current_session_id, encrypted_content, keyword_hashes, int(time.time()))
            )
            self.db_manager.execute_query(
                "INSERT INTO document_access (doc_id, user_id, session_id, encrypted_key) VALUES (?, ?, ?, ?)",
                (doc_id, self.user_id, self.current_session_id, encrypted_key)
            )
            self.db_manager.commit()
            logger.info(f"Document {doc_id} added to session {self.current_session_id}.")
            ...
            for position, trigram in enumerate(trigrams):
                ...
                self.db_manager.execute_query(
                    """INSERT OR REPLACE INTO keyword_trigrams 
                       (doc_id, session_id, trigram_hash, encrypted_position) VALUES (?, ?, ?, ?)""",
                    (doc_id, self.current_session_id, trigram_hash, encrypted_position)
                )
    ...
    def search_documents(self, keyword: str, include_archived: bool = False) -> List[str]:
        ...
        if include_archived and self.archive_access_key:
            archived_sessions = self.db_manager.execute_query(
                "SELECT session_id FROM sessions WHERE user_id = ? AND session_id != ? AND is_active = 0",
    ...
    def _search_in_session(self, keyword: str, session_id: str, session_key: bytes) -> List[str]:
        ...
        results = self.db_manager.execute_query(
            """SELECT d.doc_id, d.keyword_hashes FROM documents d
               ...
               WHERE d.session_id = ? AND a.user_id = ?""",
            (session_id, self.user_id)
        )
        ...
        for trigram in trigrams:
            ...
            results = self.db_manager.execute_query(
                """SELECT DISTINCT kt.doc_id 
                   ...
                   WHERE kt.session_id = ? AND kt.trigram_hash = ? AND da.user_id = ?""",
    ...
    def get_document(self, doc_id: str) -> Optional[str]:
        ...
        access_info = self.db_manager.execute_query(
            "SELECT session_id, encrypted_key FROM document_access WHERE doc_id = ? AND user_id = ?",
            (doc_id, self.user_id)
        )
        ...
        if include_archived and self.archive_access_key:
            ...
            results = self.db_manager.execute_query(
                """SELECT da.doc_id, da.session_id, d.created_at, s.is_active
                   ...
                   JOIN documents d ON da.doc_id = d.doc_id
                   JOIN sessions s ON da.session_id = s.session_id
                   WHERE da.user_id = ? AND da.session_id = ?
                   ORDER BY d.created_at DESC""",
    ...
    def delete_document(self, doc_id: str):
        ...
        access = self.db_manager.execute_query(
            "SELECT doc_id, session_id FROM document_access WHERE doc_id = ? AND user_id = ?",
            (doc_id, self.user_id)
        )
        if not access:
    ...
    def get_session_info(self) -> Dict[str, Any]:
        ...
        sessions = self.db_manager.execute_query(
            """SELECT session_id, created_at, is_active,
                      (SELECT COUNT(*) FROM documents WHERE session_id = s.session_id) as doc_count
               FROM sessions s
               WHERE user_id = ?
...
def main():
    ...
    server_url = input("Enter server URL (e.g., http://your-ubuntu-ip:1111): ")
    ...
    if user_type not in ["new", "existing"]:
